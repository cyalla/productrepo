...
public ProductDTO createProduct(@RequestBody ProductDTO productDTO) {
    try {
        ...
        // Check if price and tax are empty or null
        if (productDTO.getPrice() == null || productDTO.getPrice().isEmpty() ||
            productDTO.getTax() == null || productDTO.getTax().isEmpty()) {
            throw new IllegalArgumentException("Price and tax cannot be empty or null");
        }
        ...
        logger.error("Illegal argument exception occurred: {}", e.getMessage());
        return new ErrorDTO("Invalid input"); 
    } catch (Exception e) {
        logger.error("An error occurred: {}", e.getMessage());
        return new ErrorDTO("An error occurred during product creation");
    }
}
    @GetMapping("/{id}")
