...
public ProductDTO createProduct(@RequestBody ProductDTO productDTO) {
    try {
        if (productDTO == null || productDTO.isEmpty()) {
            throw new IllegalArgumentException("Invalid productDTO input.");
        }
        ...
        if (productDTO.getPrice() == null || productDTO.getPrice().isEmpty()) {
            throw new IllegalArgumentException("Invalid price. Price cannot be empty.");
        }
        double priceDouble = Double.parseDouble(productDTO.getPrice());
        product.setPrice(priceDouble);

        if (productDTO.getTax() == null || productDTO.getTax().isEmpty()) {
            throw new IllegalArgumentException("Invalid tax. Tax cannot be empty.");
        }
        ...
        if (savedProduct == null) {
            throw new IllegalArgumentException("Failed to create the product.");
        }
        ...
        throw new IllegalArgumentException("Error parsing double value.", e);
    } catch (IllegalStateException | IllegalArgumentException ex) {
        throw ex;
    } catch (Exception e) {
        throw new IllegalArgumentException("An error occurred while creating the product.", e);
    }
}
